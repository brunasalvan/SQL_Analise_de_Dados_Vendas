...
SELECT TOP 1 COUNT(PRODUCT_CATEGORY_NAME) AS VENDAS_PRODUTO, PRODUCT_CATEGORY_NAME
FROM Tb_ACT_OLIST_ORDER_ITEMS AS OI
	INNER JOIN Tb_ACT_OLIST_PRODUCTS AS P
...
FROM Tb_ACT_OLIST_PRODUCTS AS P
	INNER JOIN Tb_ACT_OLIST_ORDER_ITEMS AS OI
	ON P.PRODUCT_ID = OI.PRODUCT_ID
	INNER JOIN Tb_ACT_OLIST_SELLERS AS S
	ON OI.SELLER_ID = S.SELLER_ID
GROUP BY P.PRODUCT_CATEGORY_NAME, S.SELLER_STATE
ORDER BY SELLER_STATE, COUNT(PRODUCT_CATEGORY_NAME) DESC;


/* Produtos mais vendidos em determinado estado */
...
/* Quais estados estão abaixo da média total de compras? (TEM QUE TER OUTRO MODO) */

CREATE VIEW COMPRAS_POR_ESTADO AS
SELECT CUSTOMER_STATE AS ESTADO, COUNT(CUSTOMER_STATE) AS TOTAL_COMPRAS
FROM Tb_ACT_OLIST_CUSTOMER
GROUP BY CUSTOMER_STATE;

SELECT ESTADO, TOTAL_COMPRAS
FROM COMPRAS_POR_ESTADO
WHERE TOTAL_COMPRAS < (SELECT AVG(TOTAL_COMPRAS) FROM COMPRAS_POR_ESTADO)
...
/* Listar apenas pedidos entregue fora do prazo de entrega */
